...
# Homelab Node Specs Gatherer
# Simple bash script to gather neofetch data from homelab nodes
...
selected=0
verbose_mode=0
menu_items=("View All Specs" "Check Node Status" "Add Node" "Onboard Node" "Remove Node" "Export Specs" "View Live Stats" "Toggle Verbose Mode" "Run Tests" "Exit")

show_menu() {
    clear
    echo -e "${BLUE}=== Homelab Specs Gatherer ===${NC}"
...
run_tests() {
    ...
    local test_script="./test-homelab-specs.sh"

    # Check if test script exists in current directory
    if [ ! -f "$test_script" ]; then
        # Try the same directory as this script
        local script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
        test_script="$script_dir/test-homelab-specs.sh"
    fi

    if [ ! -f "$test_script" ]; then
        echo -e "${RED}Error: test-homelab-specs.sh not found${NC}"
