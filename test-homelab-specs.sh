...
# Test Suite for Homelab Specs Gatherer
...
show_menu() {
    clear
    echo -e "${BLUE}=== Homelab Specs Tester ===${NC}"
    echo ""
...
test_menu_structure() {
    ...
    local script="$script_dir/homelab-specs.sh"

    # Check if main script exists
    if [ ! -f "$script" ]; then
        print_result "Main script existence" "FAIL" "homelab-specs.sh not found at $script"
        echo ""
        read -p "Press Enter to continue..."
        return
    fi
